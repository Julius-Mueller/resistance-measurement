...
#			self.chooseCryo()
	...
	def testIPPort(self):
		...
		try:
			self.device = Model336(ip_address=self.cryo_IP)
	...
	def chooseCryo(self):
		""" Opens GUI window for the user to input IP address connect to his Lake Shore Model 336 controller. """
		...
		e22 = tk.Button(self.chooseWindow, text='test', command=self.testIPPort)
		e22.grid(row=2, column=2, padx=10)

		self.testVar = tk.StringVar()
		self.testIPPort()
		e23 = tk.Label(self.chooseWindow, textvariable=self.testVar)
		e23.grid(row=2, column=3, padx=10)

		e32 = tk.Button(self.chooseWindow, text='OK', command=self.chooseAndClose)
		e32.grid(row=3, column=2, padx=10)
